...
## 퀘스트 시스템
  ...
  - 퀘스트 저장/불러오기, 상태 동기화
## UI/UX
  - 게임의 분위기에 맞춘 전체 UI / 레이아웃 통일

  - 탭 메뉴를 만들어 게임 내부의 전체적인 메뉴 팝업과 그 안의 컨텐츠 관리
...
## 사운드 구성
  - 씬 별 지정된 BGM 을 씬 이동을 감지하여 지정된 BGM을 재생 및 관리

  - 오디오 믹서를 사용한 설정의 오디오 볼륨 조절 기능

  - SFX 매니저를 통한 환경음 동작 기능 (플레이어가 걷는 소리, 버튼음, havest 행위 등)

## 주요구성
### 1. 튜토리얼
  - 조작키, 농사, 스태미나, QTE 미니게임, 인벤토리, 도감, 시스템, 낚시, 상호작용, 상점, 퀵슬롯, 물뿌리개, 지도, 판매상자, 달리기, 요리, 축제, 강제취침시간의 튜토리얼이 존재합니다.
  - 적절한 상황에서 튜토리얼 팝업이 생성되며, 집앞 게시판에서 다시 확인이 가능합니다.
### 2. 트랜지션
  - 화면 또는 맵 전환시, 로딩 화면 출력 후 전환
### 3. 드롭 아이템
  - 아이템이 필드에 드롭되면, 충분이 거리가 가까워졌을때 플레이어 캐릭터에게로 당겨져 인벤토리에 추가됩니다.
